...
class TextField extends View<TextFieldAttr>
{
    ...
    function render(attr:TextFieldAttr)
    {
        return @hxx '<div class=${attr.className.add(["mdc-text-field" => true,
                        ...
                        <else>
                            <input type=${attr.type != null ? attr.type : "text"} class="mdc-text-field__input" id=${"tf" + textFieldId} value=${attr.value} required=${attr.required} onchange={attr.onedit.invoke(event.target.value)}/>
                        </if>
                        <if ${attr.label != null}>
